   ...
   def scrape_page
     ...
     albums.each do |album|
       ...
       album.css("ul.breadcrumb").each do |breadcrumb|
         ...
         @work_subcategory = Subcategory.find_or_create_by(:name => subcategory, :category_id => self.category.id)
         binding.pry
         Work.find_or_create_by(:title => title, :image_url => "https://www.henle.de#{image_link}", :link_url => "https://www.henle.de#{showpage_link}", :composer_id => self.id, :subcategory_id => @work_subcategory.id)
       end
     end
   end
end
